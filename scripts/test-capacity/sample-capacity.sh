...
# container memory (MiB), container count, sum of container CPU % (100 = one
# core), disk MB/s. The alpine probe is not a gate container (slot.go
# gateContainerPatterns: dolt/testcontainers/ryuk).
set -uo pipefail
out="${1:?outdir}"; mkdir -p "$out"; f="$out/capacity.tsv"
[[ -s "$f" ]] || printf 'epoch\tload1\tvm_total_kb\tvm_avail_kb\tswap_total_kb\tswap_free_kb\tctr_mem_mib\tctr_count\tctr_cpu_pct\tdisk_mbps\n' > "$f"
while :; do
