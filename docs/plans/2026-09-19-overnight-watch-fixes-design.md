...
**D3. Automatic rebuild between gates.** Three pieces. First, an in-flight review marker: `editorial.Run` takes a slot-style role `<rig>/om-review` (flock plus owner metadata from `internal/slot`, no pool accounting) for the duration of the review, which makes reviews visible to `gt slot status` and to the batch path. Second, `gt rebuild` refuses, with a reason, while any `<rig>/refinery`, `<rig>/refinery-batch`, or `<rig>/om-review` role is held, and retries for up to ten minutes when called with `--wait`. Third, the daemon handler (`handler.go:281-296`) evaluates the `condition` gate type by running the plugin's `check` command; the rebuild-gt plugin becomes `type = "condition"` with `check = "gt stale --json | jq -e .stale"` and a 1h cooldown as well. Un-park the deployed plugin as part of the rollout. Test: handler unit test for the condition branch and its run record; `gt rebuild` refusal test with a held slot. Owner: convoy; operator un-parks.
...
**Phase 0, by hand, in one maintenance window today.** `gt down`; install the gt binary from main (carries gt-akap); fix the formula Step 2 text in the repo and the town-tier copy; set `batch_min_age` 20m and `batch_max` 6; refresh the town-tier witness formula; edit the witness template and re-render; `scheduled_maintenance.enabled` false; write the dispatch and rubric directives; set `max_local` 3 and the beads `prime.max-memory-chars`; `gt up`. Then watch the first batch run (gt-9z48 records it).
