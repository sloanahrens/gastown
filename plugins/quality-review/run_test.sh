...
test_note_with_no_worker_is_counted() {
  echo ""
  echo "=== note with no worker ==="
  new_case
...
JSON
  ...
  run_plugin "$CASE_DIR/out"
  assert_eq "$PLUGIN_RC" "0" "note with no worker: exits 0"
  assert_file_contains "$TEST_STATE/receipts.log" "1 unattributable" \
    "note with no worker: counted in the receipt, not dropped silently"
}
...
JSON
  ...
  assert_file_contains "$TEST_STATE/receipts.log" "--result success" "parked rigs: records a receipt"
}
