...
// MR up immediately).
//
// taskIDs is a candidate list, not one id, because gt done cannot assume which
// bead names the conflict task. issueID is derived from the branch name unless
// --issue is passed, and a conflict-resolution polecat may still be sitting on
...
// task. The agent bead's hook_bead names the bead the polecat was actually
// slung, which is the task in that case, so the call site passes both and the
