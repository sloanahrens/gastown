...
func runPatrolScan(cmd *cobra.Command, args []string) error {
	...
	if patrolScanJSON {
		return outputPatrolScanJSON(rigName, timestamp, zombieResult, stallResult, completionResult, activityResult, receipts)
	}

	return outputPatrolScanHuman(rigName, zombieResult, stallResult, completionResult, activityResult, receipts)
}
...
func outputPatrolScanJSON(rigName, timestamp string, zombieResult *witness.DetectZombiePolecatsResult, stallResult *witness.DetectStalledPolecatsResult, completionResult *witness.DiscoverCompletionsResult, activityResult []witness.RealActivity, receipts []witness.PatrolReceipt) error {
...
func outputPatrolScanHuman(rigName string, zombieResult *witness.DetectZombiePolecatsResult, stallResult *witness.DetectStalledPolecatsResult, completionResult *witness.DiscoverCompletionsResult, activityResult []witness.RealActivity, _ []witness.PatrolReceipt) error {
	...
	if zombieCount == 0 && stallCount == 0 && completionCount == 0 {
		...
		fmt.Printf("Summary: %d zombie(s) (%d active-work), %d stall(s), %d completion(s), %d activity candidate(s)\n",
			zombieCount, activeCount, stallCount, completionCount, staleCandidates)
	}
