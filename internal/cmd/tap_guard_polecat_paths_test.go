...
// fakePolecatTown builds a minimal on-disk town tree at town, mirroring the
// real layout so the path classifier can be exercised against a real structure.
func fakePolecatTown(t *testing.T, town string) string {
	t.Helper()
	for _, d := range []string{
		"mayor",
		"logs",
		".dolt-data",
		filepath.Join("gastown", "polecats", "opal", "gastown", "internal"),
		filepath.Join("gastown", "polecats", "onyx", "gastown"),
		filepath.Join("gastown", "crew"),
		filepath.Join("gastown", "refinery", "rig"),
		filepath.Join("gastown", "witness"),
		filepath.Join("gastown", "mayor", "rig"),
		filepath.Join("gastown", "settings"),
		filepath.Join("gastown", ".repo.git"),
	} {
...
func polecatFakeTown(t *testing.T) (town, rig, worktree string) {
	town = filepath.Join(t.TempDir(), "gt")
	fakePolecatTown(t, town)
	rig = filepath.Join(town, "gastown")
	worktree = filepath.Join(rig, "polecats", "opal", "gastown")
	return
}

// runTapGuardPolecatPaths drives the guard the way Claude Code does — hook-input
// JSON on stdin, cwd inside a polecat worktree — so the whole path is covered
// at once. Returns stderr output for inspection.
func runTapGuardPolecatPaths(t *testing.T, hookInput string, cwd string) string {
	t.Helper()
	t.Chdir(cwd)

	var guardErr error
	var stderr strings.Builder
	stderrWriter := captureStderrWriter(t, func() {
		withStdin(t, hookInput, func() {
			guardErr = runTapGuardPolecatPaths(tapGuardPolecatPathsCmd, nil)
		})
	})
	stderr.WriteString(stderrWriter.String())
...
func TestEditWriteBlocksOutsideWorktree(t *testing.T) {
	town, rig, worktree := polecatFakeTown(t)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			switch tt.tool {
			case "NotebookEdit":
				input = fmt.Sprintf(`{"tool_name":"%s","tool_input":{"notebook_path":"%s"}}`, tt.tool, tt.path)
			default:
			...
			if tt.blocked {
				if guardErr == nil {
					t.Errorf("%s(%q) was allowed, expected blocked; stderr: %s", tt.tool, tt.path, stderr.String())
				} else if !isSilent || code != 2 {
					t.Errorf("%s(%q) blocked with err=%v (silent=%v code=%d), want silent exit 2; stderr: %s",
						tt.tool, tt.path, guardErr, isSilent, code, stderr.String())
				}
				if !strings.Contains(stderr.String(), "CROSS-WORKTREE EDIT BLOCKED") {
					t.Errorf("%s(%q) blocked without CROSS-WORKTREE banner; stderr: %s",
						tt.tool, tt.path, stderr.String())
				}
			} else {
				if guardErr != nil {
					t.Errorf("%s(%q) was blocked (%v, silent=%v), expected allowed; stderr: %s",
						tt.tool, tt.path, guardErr, isSilent, stderr.String())
				}
			}
		})
	}
...
func TestBashBlocksWriteCapableTargets(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.blocked {
				if guardErr == nil {
					t.Errorf("command %q was allowed, expected blocked; stderr: %s", tt.command, stderr.String())
				} else if !isSilent || code != 2 {
					t.Errorf("command %q blocked with err=%v (silent=%v code=%d), want silent exit 2; stderr: %s",
						tt.command, guardErr, isSilent, code, stderr.String())
				}
			} else {
				if guardErr != nil {
					t.Errorf("command %q was blocked (%v, silent=%v), expected allowed; stderr: %s",
						tt.command, guardErr, isSilent, stderr.String())
				}
			}
		})
	}
...
func TestNonPolecatContextNotBlocked(t *testing.T) {
	...
	if guardErr != nil {
		t.Errorf("crew Edit to rig root was blocked (%v), expected allowed", guardErr)
	}
	...
	if guardErr != nil {
		t.Errorf("crew Bash python to rig root was blocked (%v), expected allowed", guardErr)
	}
}
