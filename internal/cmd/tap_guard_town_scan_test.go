...
// search pattern is not a scan root, and skipping the pattern must not skip
...
//
// The bypasses the skip could open, and which the blocked cases here pin: fd
// reads a lone positional argument that names an existing path as its search
// root rather than its pattern (`fd /`, `fd $HOME`), fd's path-taking flags
// and rg --files carry paths instead of a pattern, and a pattern escaped with
// "--" must not hand the pattern slot to the root argument behind it.
func TestMatchesUnboundedScanPatternNotRoot(t *testing.T) {
	...
	tests := []struct {
		...
		// fd alone in a cwd with no such entry: nothing is a root here.
