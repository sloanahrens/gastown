...
// remotes spends this budget once per remote.
...
type RemoteDivergence struct {
	// Remote lists every configured Dolt remote that was checked, joined by
	// ",", or "" when the database has no remote at all. An empty Remote
	// means there was nothing to verify — it is not a verdict that the
	// database is safe. When Diverged is true, or an error is returned
	// alongside a non-empty Remote, Remote instead names the single remote
	// responsible, since the check stops at the first problem it finds.
	Remote string
...
func FetchAndVerify(ctx context.Context, db *sql.DB, dbName string) (RemoteDivergence, error) {
	...
	return RemoteDivergence{Remote: strings.Join(remotes, ",")}, nil
}
