...
// TestResolveMergeQueueConfigThreeTierPrecedence reproduces gt-egiv: five
// call sites each duplicated the rig-root -> repo -> rig-local merge-queue
// merge, and one of them (buildRefineryPatrolVars) skipped the rig-root
// floor entirely. ResolveMergeQueueConfig is the single resolver all call
// sites must route through, so this test pins the merge order directly
// against it rather than against any one caller.
func TestResolveMergeQueueConfigThreeTierPrecedence(t *testing.T) {
	t.Parallel()
	rigPath := t.TempDir()
  ...
  "merge_queue": {
	...
	mq := ResolveMergeQueueConfig(rigPath, repoRoot)
	if mq == nil {
		t.Fatal("expected non-nil MergeQueueConfig")
	}
	if mq.BuildCommand != "make build-local" {
		t.Errorf("BuildCommand = %q, want %q (rig-local wins over repo and root)", mq.BuildCommand, "make build-local")
	}
	...
	if mq.LintCommand != "make lint-root" {
		t.Errorf("LintCommand = %q, want %q (neither repo nor local set it — falls to root floor)", mq.LintCommand, "make lint-root")
	}
}

// TestResolveMergeQueueConfigRigRootAbsent is the negative case flagged in
// gt-egiv finding 2: when the rig root has no merge_queue section at all
// (the pre-gt-me9t shape), the two-source repo/local merge must behave
// exactly as it did before the rig-root floor was introduced.
func TestResolveMergeQueueConfigRigRootAbsent(t *testing.T) {
	t.Parallel()
	rigPath := t.TempDir()
	repoRoot := filepath.Join(rigPath, "mayor", "rig")
	gastownDir := filepath.Join(repoRoot, ".gastown")
	if err := os.MkdirAll(gastownDir, 0o755); err != nil {
		t.Fatalf("mkdir %s: %v", err, err)
	}
  ...
  "merge_queue": {"test_command": "make test-repo"}
	...
	if mq.TestCommand != "make test-repo" {
		t.Errorf("TestCommand = %q, want %q (two-source merge unaffected by absent rig-root floor)", mq.TestCommand, "make test-repo")
	}
}

// TestResolveMergeQueueConfigAllAbsent verifies the resolver returns nil,
// matching MergeSettingsCommand(nil, nil), when no layer configures anything.
func TestResolveMergeQueueConfigAllAbsent(t *testing.T) {
	t.Parallel()
	rigPath := t.TempDir()
	mq := ResolveMergeQueueConfig(rigPath, filepath.Join(rigPath, "mayor", "rig"))
	if mq != nil {
		t.Errorf("expected nil MergeQueueConfig, got %+v", mq)
	}
}
